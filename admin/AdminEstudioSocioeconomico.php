...
class AdminEstudioSocioeconomico extends Admin {
    ...
    private function construirEstudioSocioeconomico($data) {
        return new EstudioSocioeconomico($data["idEmprendedor"],
                ...
                $data["resultadoVisita"], $data["trabajadorSocial"], $data["fechaCreacion"] ?? "");
    }
    ...
    private function construirSeccionEmpleabilidad(array $data) {
        ...
        return new EstudioSocioeconomicoSeccionEmpleabilidad($empresaActual, $puestoActual,
                $antiguedadActual, $empresaAnterior, $puestoAnterior, $antiguedadAnterior,
                $motivoRetiro, $cuentaConSeguroSocial);
    }
    ...
    private function construirSeccionEconomiaFamiliar(array $data) {
        return new EstudioSocioeconomicoSeccionEconomiaFamiliar($data["ingresoMensual"],
                $data["egresos"]);
    }

    private function construirSeccionVivienda(array $data) {
        return new EstudioSocioeconomicoSeccionVivienda($data["tipoVivienda"],
                $data["condicionVivienda"], $data['familiasHabitantesVivienda'],
                $data["usoVivienda"], $data["piso"] ?? [], $data["techo"] ?? [], $data["pared"] ?? [],
                $data["distribucion"] ?? [], $data["servicios"] ?? []);
    }
    ...
    private function construirSeccionConclusiones(array $data) {
        return new EstudioSocioeconomicoSeccionConclusiones($data["actitudesPositivas"] ?? [],
