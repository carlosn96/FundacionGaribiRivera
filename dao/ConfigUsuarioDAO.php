...
class ConfigUsuarioDAO extends DAO {
    ...
    private function actualizar_config_usuario($id_usuario, $key_gpt = null, $key_huggingface = null, $id_plantel_actual = null, $id_carrera_actual = null, $id_ciclo_escolar = null) {
    ...
    public function recuperar_configuracion_carrera_plantel($id_coord) {
        ...
        $prep = new PreparedStatmentArgs();
        $prep->add("i", $id_usuario);
        if (($rs = $this->ejecutar_instruccion_prep_result("SELECT id_carrera_actual, id_plantel_actual, id_ciclo_escolar_actual FROM config_usuario WHERE id_usuario = ?", $prep))) {
            return $rs[0];
        } else {
            return ["id_plantel_actual" => null, "id_carrera_actual" => null, "id_ciclo_escolar_actual" => null];
        }
    }
}
