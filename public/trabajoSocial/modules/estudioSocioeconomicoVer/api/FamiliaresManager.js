...
function calculateTotal(fixed, variable) {
  const fixedAmount =
    typeof fixed === "string"
      ? parseFloat(fixed.replace(/[^0-9.-]+/g, ""))
      : Number(fixed) || 0;
  const variableAmount =
    typeof variable === "string"
      ? parseFloat(variable.replace(/[^0-9.-]+/g, ""))
      : Number(variable) || 0;
...
function generateFamilyTable(data) {
  ...
  data.forEach((familiar) => {
    const total = calculateTotal(
      familiar.ingresoMensualFijo,
      familiar.ingresoMensualVariable
    );
    ...
    row.innerHTML = `
            <td class="ps-4">
                <div>
                    ...
                    <small class="text-muted d-md-none edad">${
                      familiar.edad
                    } años</small>
                </div>
            </td>
            <td class="text-center">
                <span class="badge bg-light text-dark edad">${
                  familiar.edad
                }</span>
            </td>
            <td>
                <span class="badge ${getParentescoBadge(
                  familiar.parentesco
                )} parentesco">${familiar.parentesco}</span>
            </td>
            <td class="d-none d-md-table-cell">
                <small class="text-muted estado-civil">${
                  familiar.estadoCivil.value
                }</small>
            </td>
            <td class="d-none d-lg-table-cell">
                <small class="text-muted escolaridad">${
                  familiar.escolaridad.value
                }</small>
            </td>
            <td class="d-none d-lg-table-cell">
                <small class="text-muted ocupacion">${
                  familiar.ocupacion.value
                }</small>
            </td>
            <td class="text-end">
                <div class="d-flex flex-column align-items-end">
                    <span class="fw-bold text-success ingreso-total">${formatCurrency(
                      total
                    )}</span>
...
function updateStats(data) {
  ...
  data.forEach((familiar) => {
    const fijo =
      typeof familiar.ingresoMensualFijo === "string"
        ? parseFloat(familiar.ingresoMensualFijo.replace(/[^0-9.-]+/g, "")) || 0
        : Number(familiar.ingresoMensualFijo) || 0;

    const variable =
      typeof familiar.ingresoMensualVariable === "string"
        ? parseFloat(
            familiar.ingresoMensualVariable.replace(/[^0-9.-]+/g, "")
          ) || 0
  ...
  if (average > 0) {
    ...
    if (average < montoVulnerable) {
      ...
      $("#esVulnerable").removeClass().addClass("badge bg-success").text("No");
    }
  } else {
...
function sortTable(columnIndex) {
  ...
  currentSortDirection = isNewColumn
    ? "asc"
  ...
  modalContent.innerHTML = `
        <div class="row g-3">
            <div class="col-12 text-center mb-3">
                ...
                <small class="text-muted">${familiar.edad} años • ${
    familiar.parentesco
  }</small>
            </div>
            ...
            <div class="col-12">
                ...
                <div class="row">
                    <div class="col-6">
                        ...
                        <p class="mb-0 fw-medium">${formatCurrency(
                          familiar.ingresoMensualFijo
                        )}</p>
                    </div>
                    <div class="col-6">
                ...
                <div class="mt-2 pt-2 border-top">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="fw-bold text-success fs-5">${formatCurrency(
                          total
                        )}</span>
                    </div>
                </div>
...
function editMember(id) {
  mostrarMensajeInfo(
    "En este momento solo se puede eliminar un familiar de los agregados previamente, la edición de informacion sigue en desarrollo.",
    false
  );
  /* const familiar = familyData.find(f => f.idFamiliar === id);
     if (!familiar) return;
...
$("#guardarCambiosFamiliar").on("click", function () {
  ...
  familiar.ingresoMensualVariable =
    parseFloat($("#editIngresoVariable").val()) || 0;
...
function addNewMember() {
  alert(
    "Esta función abriría un modal de Bootstrap para agregar un nuevo familiar."
  );
}
