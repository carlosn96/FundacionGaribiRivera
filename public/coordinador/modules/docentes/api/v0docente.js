...
function ready() {
    $(document).ready(function () {
        ...
        $('#profesorForm').submit(function (event) {
            ...
            if (elementosMateria.length > 0) {
                crearPeticion(urlAPI, {
                    ...
                    data: $(this).serialize() + "&" + obtenerParametrosCarreraPlantelCiclo()
                }, print);
            } else {
        ...
        recuperarCarreras(function () {
            let carrera = $("#selectorCarrera").find('option:selected').text();
            let plantel = $("#selectorPlantel").find('option:selected').text();
            let ciclo = $("#selectorCicloEscolar").find('option:selected').text();
            const data = {
                case: "recuperar_docentes",
                data: obtenerParametrosCarreraPlantelCiclo()
            };
            $("#nombreCarrera").text(carrera + " del Plantel " + plantel + " (Ciclo escolar " + ciclo + ")");
...
function crearListaProfesores(data) {
    ...
    if (Object.keys(data).length) {
        ...
        let ciclo = $("select[id=selectorCicloEscolar] option:selected").text();
        accordion.append($(`<div class="alert alert-warning" role="alert">
                    No se han agregado docentes para el ciclo escolar ${ciclo}
                  </div>`));
    }
}
...
function obtenerParametrosCarreraPlantelCiclo() {
    let carrera = $("#selectorCarrera").find('option:selected').val();
    let plantel = $("#selectorPlantel").find('option:selected').val();
    let ciclo = $("#selectorCicloEscolar").find('option:selected').val();
