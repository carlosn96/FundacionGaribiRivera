...
export const RadioGroupComponent: React.FC<RadioGroupProps> = ({
  ...
  return (
    <div className={cn("space-y-4", className)} hidden={hidden}>
      ...
      <RadioGroup value={value} onValueChange={onChange} name={name} className="space-y-3">
        {options.map((option) => (
          <div
            key={option[idKey]}
          ...
          >
            <RadioGroupItem
              ...
              className="mt-0.5 h-5 w-5 text-primary focus:ring-primary/20 border-border group-hover:border-primary/50 transition-all duration-200"
            />
            <Label
